...
final class ImageListViewModel: ImageListViewModelProtocol {
    ...
    func onImageTap(at index: Int) {
        guard case let .loaded(models) = state, let model = models.elementOrNil(at: index) else {
            return
        }

        do {
            try eventsHandler(.onImageTap(model.detailImageURL))
        }
        catch {
            debugPrint(error)
        }
...
private extension ImageListViewModel {
    func requestInitialSetOfItems() async {
        ...
        do {
            ...
            let models = fetchedImages.compactMap { factory.makeViewModel(from: $0) }
    ...
    func paginateMoreItemsIfNeeded(index: Int) async {
        ...
        guard case let .loaded(currentModels) = state, !updateState.isPaginating else {
            return
        }
    ...
    func updateLike(for index: Int) async {
        ...
        guard case var .loaded(models) = state, let model = models.elementOrNil(at: index) else {
            return
        }
